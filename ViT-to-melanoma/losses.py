...
def AAAI_sdf_loss(net_output, gt):
    ...
    L_SDF_AAAI = -L_product.mean() + torch.norm(net_output - gt_sdm, 1) / torch.numel(
        net_output
    )
...
def sdf_kl_loss(net_output, gt):
    ...
    with torch.no_grad():
        ...
        gt_sdf = (
            torch.from_numpy(gt_sdf_npy + smooth).float().cuda(net_output.device.index)
        )
...
class DiceLoss(nn.Module):
    ...
    def forward(self, inputs, target, weight=None, sigmoid=False):
        ...
        for i in range(0, self.n_classes):
            # print("inputs[:, i]", inputs[:, i].shape, ", target[:, i]", target[:, i].shape)
