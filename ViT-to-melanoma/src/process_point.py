    ...
    if contour_len == 0:
        ...
        for contour in contours:
            ...
            for i in range(len(points)):
                ...
                neighbor_points_index[np.where(neighbor_points_index < 0)[0]] += len(
                    points
                )
                neighbor_points_index[
                    np.where(neighbor_points_index > len(points) - 1)[0]
                ] -= len(points)
                if stds[i] < np.min(stds[neighbor_points_index]) or stds[i] > np.max(
            ...
            rr, cc = skimage.draw.polygon(
                np.array(selected_points)[:, 1], np.array(selected_points)[:, 0]
            )
            maskk[rr, cc] = 1
...
def point_gen_isic2016():
    ...
    for split in ["Train", "Test", "Validation"]:
        data_dir = "./processed/isic2016/{}/Label".format(split)
