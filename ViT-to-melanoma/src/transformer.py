...
from .BAT_Modules import (
    BoundaryWiseAttentionGate1D,
    BoundaryWiseAttentionGate2D,
    BoundaryWiseAttentionGateAtrous1D,
    BoundaryWiseAttentionGateAtrous2D,
)
...
class BoundaryAwareTransformer(nn.Module):
    def __init__(
        ...
        encoder_norm = nn.LayerNorm(d_model) if normalize_before else None
        self.encoder = BoundaryAwareTransformerEncoder(
            point_pred_layers, encoder_layer, num_encoder_layers, encoder_norm
        )
        decoder_layer = TransformerDecoderLayer(
            d_model, nhead, dim_feedforward, dropout, activation, normalize_before
        )
    ...
    def forward(self, src, mask, query_embed, pos_embed):
        ...
        tgt = torch.zeros_like(query_embed)
        memory, weights = self.encoder(
            src, src_key_padding_mask=mask, pos=pos_embed, height=h, width=w
        )
...
class BoundaryAwareTransformerEncoder(nn.Module):
    def __init__(self, point_pred_layers, encoder_layer, num_layers, norm=None):
        super().__init__()
